...
/**
 * Servicio que gestiona la conexión con el microservicio de autenticación
 ...
 */
@Injectable()
export class AuthConnectionManager {
    private readonly logger = new Logger(AuthConnectionManager.name);
    private isConnected: boolean = false;
    private connectionAttempts: number = 0;
    private readonly MAX_RETRIES = 3;
    private readonly RETRY_DELAY_MS = 5000; // 5 segundos entre reintentos
    private readonly INITIAL_DELAY_MS = 2000; // 2 segundos antes del primer intento
    ...
    async attemptConnection(connectFn: () => Promise<void>): Promise<void> {
        // Esperar un poco antes del primer intento para que MongoDB esté listo
        ...
        for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
            ...
            try {
                this.logger.log(`🔄 Intento ${attempt}/${this.MAX_RETRIES} de conexión al servicio de autenticación...`);

                await connectFn();

                this.isConnected = true;
                this.logger.log(`✅ Conexión exitosa al servicio de autenticación (intento ${attempt})`);
                return;
        ...
        this.logger.error(
            `❌ No se pudo conectar al servicio de autenticación después de ${this.MAX_RETRIES} intentos. ` +
