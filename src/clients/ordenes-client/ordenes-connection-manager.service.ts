...
/**
 * Servicio que gestiona la conexión con el microservicio de órdenes
 ...
 */
@Injectable()
export class OrdenesConnectionManager {
    private readonly logger = new Logger(OrdenesConnectionManager.name);
    private isConnected: boolean = false;
    private connectionAttempts: number = 0;
    private readonly MAX_RETRIES = 3;
    private readonly RETRY_DELAY_MS = 5000; // 5 segundos entre reintentos
    private readonly INITIAL_DELAY_MS = 4000; // 4 segundos antes del primer intento
    ...
    async attemptConnection(connectFn: () => Promise<void>): Promise<void> {
        // Esperar un poco antes del primer intento para que otros servicios estén listos
        ...
        for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
            ...
            try {
                this.logger.log(`🔄 Intento ${attempt}/${this.MAX_RETRIES} de conexión al servicio de órdenes...`);

                await connectFn();

                this.isConnected = true;
                this.logger.log(`✅ Conexión exitosa al servicio de órdenes (intento ${attempt})`);
                return;
        ...
        this.logger.error(
            `❌ No se pudo conectar al servicio de órdenes después de ${this.MAX_RETRIES} intentos. ` +
